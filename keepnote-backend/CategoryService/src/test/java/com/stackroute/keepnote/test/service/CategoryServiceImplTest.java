...
public class CategoryServiceImplTest {
    ...
    public void deleteCategory() throws Exception {
        when(categoryRepository.findById(category.getCategoryId())).thenReturn(options);
        boolean flag = categoryServiceImpl.deleteCategory(category.getCategoryId());
    ...
    public void updateCategoryTestSuccess() {
        ...
        Category fetchedCategory = categoryServiceImpl.updateCategory(category, category.getCategoryId());
    ...
    public void getCategoryByIdTestSuccess() throws CategoryNotFoundException {
        when(categoryRepository.findById(category.getCategoryId())).thenReturn(options);
        Category fetchedCategory = categoryServiceImpl.getCategoryById(category.getCategoryId());
        Assert.assertEquals(category, fetchedCategory);
    ...
    public void getCategoryByIdTestFailure() throws CategoryNotFoundException {
        when(categoryRepository.findById(category.getCategoryId())).thenThrow(NoSuchElementException.class);
        Category fetchedCategory = categoryServiceImpl.getCategoryById(category.getCategoryId());
        Assert.assertEquals(category, fetchedCategory);
